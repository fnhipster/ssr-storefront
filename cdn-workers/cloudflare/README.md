...
Cloudflare Workers adapter for AEM Edge Delivery SSR. Proxies requests to the AEM origin and enriches HTML responses with server-rendered structured data, meta tags, and block HTML for SEO and LLM consumption.
...
Complete the standard AEM Edge Delivery Cloudflare worker setup before enabling SSR:

1. Follow the [AEM Cloudflare Worker Setup guide](https://www.aem.live/docs/byo-cdn-cloudflare-worker-setup) to:
   - Create a Cloudflare Worker and attach it to your domain route (e.g. `www.mydomain.com/*`)
   - Set `ORIGIN_HOSTNAME` to your AEM origin (e.g. `main--{site}--{org}.aem.live`)
   - Configure DNS, SSL/TLS, and caching per the guide
2. Replace the worker script with `src/index.mjs` from this package — it is a drop-in replacement that adds the SSR injection pipeline on top of the standard proxy behaviour.
...
| `ORIGIN_AUTHENTICATION` | (optional) site token for token-based authentication (starts with `hlx_`) |
...
`src/index.mjs` is a standard AEM Edge Delivery Cloudflare Worker extended with an SSR injection pipeline. For every HTML response, it runs each registered injector in sequence. An injector only activates when its template meta tag matches; all other responses pass through unchanged.
